...
namespace WebDAV
{
		...
		Client::info(const std::string& remote_resource) const noexcept
	{
		...
		for (auto response : responses)
		{
			...
			char* unescaped = curl_unescape(encode_file_name.c_str(), static_cast<int>(encode_file_name.length()));
			if (!unescaped) continue;
			std::string resource_path{unescaped};
			curl_free(unescaped);
